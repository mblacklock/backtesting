...
def compute_stats(
    ...
    if isinstance(trades, pd.DataFrame):
        ...
        trades_df = pd.DataFrame({
            ...
            # MB - Add stop loss tag
            'StopLoss': [t.tag[2] for t in trades],
    ...
    #s.loc['Return [R]'] = (equity[-1] - equity[0]) / equity[0] * 100
    s.loc['Return [R]'] = pl.sum()
    ...
    '''
    '''
    # MB - added to combine split trades due to tapered exits
    # Need to deactivate this if size = 1
    combined_pl = pl.groupby(pl.index//2).agg('sum')
    s.loc['# Trades'] = n_trades = len(combined_pl)
    win_rate = np.nan if not n_trades else (combined_pl > 0).mean()
    s.loc['Win Rate [%]'] = win_rate * 100
    s.loc['Best Trade [R]'] = combined_pl.max()
    ...
    s.loc['Win Rate [%]'] = win_rate * 100
    s.loc['Best Trade [R]'] = pl.max()
    s.loc['Worst Trade [R]'] = pl.min()
    mean_return = pl.mean()
    s.loc['Expectancy (mean R)'] = mean_return
    s.loc['Profit Factor'] = pl[pl > 0].sum() / (abs(pl[pl < 0].sum()) or np.nan)  # noqa: E501
    s.loc['SQN'] = np.sqrt(n_trades) * pl.mean() / (pl.std() or np.nan)
    s.loc['SQN100'] = np.sqrt(100) * pl.mean() / (pl.std() or np.nan)
    '''
